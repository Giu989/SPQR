...
Options[FindIrreducibleMonomials] = {"MonomialOrder" -> DegreeReverseLexicographic,"Sort"->False};
FindIrreducibleMonomials[polySystem_,vars1_,OptionsPattern[]]:=Module[
	...
	If[OptionValue["Sort"],
		...
		vars = GroebnerBasis`DistributedTermsList[polySystem,vars1,MonomialOrder->OptionValue["MonomialOrder"],CoefficientDomain->RationalFunctions,Sort->True]//Last;
		,
		vars = vars1;
	];
	
	gb = LeadingMonomials[polySystem,vars,MonomialOrder->ToString[ord]];
