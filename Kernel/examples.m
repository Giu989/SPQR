...
solveSystem//Options
constructCompanionMatrices//Options
...
solveSystem[targetList,ideal,variables,7,"IrreducibleMonomials"->irredMonomials]


output = solveSystem[targetList,ideal,variables,8,"IrreducibleMonomials"->irredMonomials]
...
(*we now reconstruct the polynomial division using reconstructSystemOutput*)
results=reconstructSystemOutput[output]
...
(*we can also generate the companion matrices associated to the ideal with the command constructCompanionMatrices[]*)
...
constructCompanionMatrices[ideal,variables,2,irredMonomials]


cmats = constructCompanionMatrices[ideal,variables,3,irredMonomials]


(*the command constructTargetCmat[] will take the companion matrices and a target, and will construct the new target c matrix. Let us take the first polynomial from the list as an example*)
targetList // First
...
targetCmat = constructTargetCmat[targetList // First,cmats]
...
reductionWithCmats = ffPolyDiv[targetCmat,irredMonomials,"DeleteGraph"->False]
...
ffPolyDiv[targetCmat,irredMonomials,"DeleteGraph"->False,"Vector"->True]


ffPolyDiv[targetCmat,irredMonomials,"DeleteGraph"->False,"cmat"->True]
...
targetCmat = constructTargetCmat[expression,cmats]
(*targetCmat // First // FFGraphDraw*)
...
(*a few final things to note: the current recursive parser is very janky. as such, so is the piece of code that interfaces to it, namely constructTargetCmat[]*)
...
output = solveSystem[{target},pList,variables,5,"IrreducibleMonomials"->irredMonomials]
results=reconstructSystemOutput[output] // First  // Collect[#,irredMonomials,Factor]&;
results
...
cmats = constructCompanionMatrices[pList,variables,6,irredMonomials]


gpolcmat = constructTargetCmat[target,cmats]
gpolcmat // First // FFGraphDraw


results2 = ffPolyDiv[gpolcmat,irredMonomials] // Collect[#,irredMonomials,Factor]&;
results==results2
...
variableCmats = constructCompanionMatrices[ideal,variables,6,irredMonomials];


jinvCmat = constructTargetCmat[jinvariantRoots,variableCmats];


jinvCoefficients = ffPolyDiv[jinvCmat,irredMonomials,"DeleteGraph"->False] // Factor
...
variables = {x,y,c};
rowReduction = solveSystem[{c^cexp},pList,variables,8,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x,y},5}]; // AbsoluteTiming
rowReduction


reconstructed  = reconstructSystemOutput[rowReduction] // First; // AbsoluteTiming
reconstructed // Short
...
variables = {x,y,z,d};
rowReduction = solveSystem[{d^dexp},pList,variables,18,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x,y,z},14},"PrintDebugInfo"->2]; // AbsoluteTiming
rowReduction
...
out = solveSystem[{z},pList,variables,18,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x,y,z},14},"IrreducibleMonomials"->irredMonomials,"PrintDebugInfo"->2]


(*ans = reconstructSystemOutput[out]; // AbsoluteTiming*)
(*ans*)
(*variableCmats = constructCompanionMatrices[pList,variables,18,irredMonomials,"PrintDebugInfo"->2];*)
(*targ = constructTargetCmat[d^dexp,variableCmats]*)
...
systemOutput = solveSystem[{target},pList,{x1,x2,x3,x4,x0},9,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x1,x2,x3,x4},5},"PrintDebugInfo"->2]


reconstructed = reconstructSystemOutput[systemOutput,"Vector"->True] // First; // AbsoluteTiming
...
systemOutput = solveSystem[{target},pList,{x0,x2,x3,x4,x1},9,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x0,x2,x3,x4},5}]
...
systemOutput = solveSystem[{target},pList,{x0,x1,x3,x4,x2},9,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x0,x1,x3,x4},5}]
...
systemOutput = solveSystem[{target},pList,{x0,x1,x2,x4,x3},9,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x0,x1,x2,x4},5}]
...
systemOutput = solveSystem[{target},pList,{x0,x1,x2,x3,x4},9,"MonomialOrder"->DegreeLexicographic,"EliminateVariables"->{{x0,x1,x2,x3},5}]
