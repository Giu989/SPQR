...
(* ::Title:: *)
(*System Generation and Loading Benchmark*)
...
(*easy*)
If[check == "easy",
pList = {a*x+y^2+x^2*y,a*x*y+b*y^2};
variables = {x,y};
targets = {x+y+x*y^2+a*b,3+2*x^2*y};
maxW = 5;
];
(*PolynomialReduce[targets,GroebnerBasis[pList,variables],variables]//Map[Last]*)
...
maxW = 27;
targets = {x0^20(*,17*x0^20*)};
];


(* ::Section::Closed:: *)
(*System Generation Old*)
...
positions = Position[jseeds,j[a___]/;Plus@@({a}[[1;;(variables // Length)-1]])<=(maxW)-21(*0*)] // Flatten; // AbsoluteTiming
...
(*(*slow*)
equations = Table[targ[i]-targetsj[[i]],{i,1,targetsj//Length}]~Join~(Outer[(#*jseeds // Reverse)&,pListj]//Expand//Flatten // DeleteDuplicates); // AbsoluteTiming
equations//Length*)


(*(*weightMatrix = generateWeightMatrix[variables[[2;;2]],variables[[1;;1]],DegreeReverseLexicographic];*)
weightMatrix = generateWeightMatrix[variables[[5;;5]],variables[[1;;4]],DegreeReverseLexicographic];*)


(*(*extracting and sorting monomials: dumb way but works for now*)
sortedMonomials = (equations // Cases[#,_j,{0,\[Infinity]}]& // DeleteDuplicates) // ReplaceAll[j[x__]:>Times@@(variables^{x})] // Apply[Plus] // MonomialList[#,variables,weightMatrix]&; // AbsoluteTiming
sortedMonomialsj = CoefficientRules[sortedMonomials,variables][[;;,;;,1]]// Map[Apply[j],#,{2}]& // Flatten; // AbsoluteTiming
monomials = (Range[1,targetsj//Length]//Map[targ]//Reverse)~Join~sortedMonomialsj; // AbsoluteTiming*)


(*systemSparse = CoefficientArrays[equations,monomials] // Last; // AbsoluteTiming
nonZeroPositionsOLD = systemSparse // ArrayRules // Most // GroupBy[Last] // Part[#, All, All, 1]&; // AbsoluteTiming*)


(* ::Section:: *)
(*System Generation New*)
...
jseeds = createAllSeeds[variables//Length,maxW] // Map[Apply[j]]; // AbsoluteTiming
If[check == "easy",
	positions = Position[jseeds,j[a___]/;Plus@@({a}[[1;;(variables // Length)-1]])<=(maxW)] // Flatten;
];
If[check == "hard",
	positions = Position[jseeds,j[a___]/;Plus@@({a}[[1;;(variables // Length)-1]])<=(maxW)-21] // Flatten;
];
jseeds = jseeds[[positions]]; // AbsoluteTiming
(* 24361 *)
Print["Starting number of equations: ", 1+(pList//Length)*(jseeds//Length)];
...
(*(*not super fast, will improve on - not the current focus*)
...
monomials = (Range[1,targetsj//Length]//Map[targ]//Reverse)~Join~sortedMonomialsj; // AbsoluteTiming*)
jseedMonomialsOuter = Outer[Times,jseeds,monomialsInIdeal]; // AbsoluteTiming
monomials = Join[(Range[1,targetsj//Length]//Map[targ]//Reverse),(Join[jseeds,jseedMonomialsOuter] // Flatten // DeleteDuplicates) // MapApply[List]//SortBy[weightMatrix . #&]//Reverse // MapApply[j]]; // AbsoluteTiming
...
(*system generation code below: time to beat: ~1 minute*)


test=idealCoefficientMatrix -> ((jseedMonomialsOuter)//ReplaceAll[PositionIndex[monomials]])//ReplaceAll[{x_}/;IntegerQ[x]->x] // Map[Thread] // Thread; // AbsoluteTiming
testList = Table[test[[;;,1,i]] -> test[[;;,2]] // Thread // DeleteCases[#,x_/;(x//First)==0]&,{i,1,pListj // Length}]; // AbsoluteTiming
testListExtracted = Table[testList // Map[Cases[#,x_/;(x//First)==i]&] // Part[#,All,All,2]&, {i,coefficients}]; // AbsoluteTiming


associationRules = Table[Table[Table[{Range[(jseedMonomialsOuter // Length)]+(targetsj // Length)+(jseedMonomialsOuter // Length)*(k-1),i} // Transpose,{i,l[[k]]}] // Transpose // Flatten[#,1]&,{k,1,pList//Length}] // Apply[Join],{l,testListExtracted}]; // AbsoluteTiming
systemAssociation = coefficients -> associationRules // Thread // Association; // AbsoluteTiming
targetAssociation = Table[targ[i]-targetsj[[i]],{i,1,targetsj//Length}] // CoefficientArrays[#,monomials]& // Last // ArrayRules // Most // GroupBy[Last] // Part[#, All, All, 1]&; // AbsoluteTiming


nonZeroPositions = Merge[{targetAssociation,systemAssociation},Join//Apply](* // Map[Flatten[#,1]&]*); // AbsoluteTiming


(*to check this result, we have to use the new order of how the equations are provided, which is gven by*)


equationsVector=idealCoefficientMatrix . (jseedMonomialsOuter//Transpose); // AbsoluteTiming
equations = Join[Table[targ[i]-targetsj[[i]],{i,1,targetsj//Length}], equationsVector // Flatten];
equations // Length
systemSparse = CoefficientArrays[equations,monomials] // Last; // AbsoluteTiming
nonZeroPositionsCheck = systemSparse // ArrayRules // Most // GroupBy[Last] // Part[#, All, All, 1]&; // AbsoluteTiming


(*note that this sorting only sorts the order of the Keys*)
(nonZeroPositions//Sort) == (nonZeroPositionsCheck//Sort)
...
FFAlgNodeSparseSolver["rowReduce","solvedSystem",{"nonZeroes"},adjlists,monomials,"NeededVars"->(Range[1,targetsj//Length]//Map[targ]//Reverse)];
...
FFGraphEvaluate["rowReduce",{21,33,0}]; // AbsoluteTiming
...
valuesUniqueToPositions = nonZeroPositions;


(*(* FIXME: This is slow. Can we derive this in some other way? *)
{numberOfRows, numberOfCols} = valuesUniqueToPositions // RightComposition[
    Values,
    Flatten[#, 1]&,
    Transpose,
    Map[Max],
    TM["Numbers of rows and cols"],
    Identity
];*)
{numberOfRows, numberOfCols} = {(jseeds // Length)*(pListj // Length) + (targetsj // Length),monomials // Length}; // TM["Numbers of rows and cols"]


valuesUnique = valuesUniqueToPositions // Keys // TM["valuesUnique"];
indexUniqueToPosition = valuesUniqueToPositions // KeyMap[
    Position[valuesUnique, #, {1}]& /* (Part[#, 1, 1]&)
] // TM["indexUniqueToPosition"];


(*(* FIXME: optimize this? *)
(* templateIndexUnique: 1.27203 sec *)
templateIndexUnique = indexUniqueToPosition // RightComposition[
    Normal,
    Map[Thread[Rule[#[[2]], #[[1]]]]&],
    Flatten[#, 1]&,
    SparseArray[#, {numberOfRows, numberOfCols}]&,
    TM["templateIndexUnique"],
    Identity
];*)


(*{templateIndexAll, indexAll} = templateIndexUnique // todata // TM["templateIndexAll"];

takePattern = Thread[{1, indexAll}] // TM["takePattern"];*)


params = Complement[Join[pList // Variables, targets // Variables],variables]~Join~{extraparam}; // TM["findingParmas"]


triples=Flatten[KeyValueMap[Function[{k,lst},Prepend[k]/@lst],indexUniqueToPosition],1]; // AbsoluteTiming
byI=GroupBy[triples,#[[2]]&]; // AbsoluteTiming
pivots=Sort@Keys[byI]; // AbsoluteTiming
adjLists=Map[Sort@DeleteDuplicates@#[[All,3]]&,byI/@pivots]; // AbsoluteTiming


reverseIndex=Association@Flatten@KeyValueMap[Thread[#2->#1]&,indexUniqueToPosition]; // AbsoluteTiming
takePattern=MapIndexed[Thread[{#2[[1]],#1}]&,adjLists]// Map[Lookup[reverseIndex,#]&]//Flatten // Thread[{1, #}]&; // AbsoluteTiming


graphName = StringJoin["ffGraph","$",RandomInteger[10^15]//ToString];
FFDeleteGraph[graphName];
FFNewGraph[graphName,"in",params];
FFAlgRatFunEval[graphName, "uniqueNonZeroes", {"in"}, params, valuesUnique] // TM;
FFAlgTake[graphName, "take", {"uniqueNonZeroes"}, takePattern] // TM;
...
FFAlgNodeSparseSolver[
    ...
    "solvedSystem",
    {"take"},
    (*templateIndexAll["AdjacencyLists"],*)
    adjLists,
    monomials,
...
learn = FFSparseSolverLearn[graphName, monomials] // TM["learn"]
newEqnNumb=FFSparseSolverMarkAndSweepEqs[graphName, "solvedSystem"]


If[check == "easy",
	Print["Sanity check: ", FFReconstructFunction[graphName,params] == {(-a^2 b-b^2)/a^3,-(1/a),1,a b,(2 b^2)/a^2,0,0,3}];
];
If[check == "hard",
	point = {1015, 4978, 2606};
	result = {3122075551308109556,2612007353074071018,3115743277764484732,3539867399255408683,6098096746023471611,8522052616898844488,7432219665703479097,4309637774841954463,8374431629366203727,2108136809288949767,1467865998625981081,7587209705720838989,8199339099540145966,9061628287923122569,7569813816883795793,7229395882175755047,7269427984908214007,1733403903158938641,6843382918771554796,8802179489188341693};
	Print["Sanity check: ", SameQ[FFGraphEvaluate[graphName, point], result]];
];
...
(*slow: timing = 47.8915*)
equationsOld = Table[targ[i]-targetsj[[i]],{i,1,targetsj//Length}]~Join~(Outer[(#*jseeds // Reverse)&,pListj]//Expand//Flatten // DeleteDuplicates); // AbsoluteTiming
equationsOld//Length
...
(*test1 = test[[;;,1,1]] -> test[[;;,2]] // Thread // DeleteCases[#,x_/;(x//First)==0]&; // AbsoluteTiming
test1*)


(*Cases[test1,x_/;(x//First)==1][[;;,2]] // Transpose // Flatten
1//ReplaceAll[nonZeroPositions] // Part[#,;;,2]& // Part[#,2;;]&*)


(*Cases[test1,x_/;(x//First)==1][[;;,2]];
Table[{Range[%[[1]]//Length]+1,i}//Transpose,{i,%}] // Transpose // Flatten[#,1]&
Cases[test1,x_/;(x//First)==a][[;;,2]];
Table[{Range[%[[1]]//Length]+1,i}//Transpose,{i,%}] // Transpose // Flatten[#,1]&
Cases[test1,x_/;(x//First)==b][[;;,2]];
Table[{Range[%[[1]]//Length]+1,i}//Transpose,{i,%}] // Transpose // Flatten[#,1]&*)


(*test2 = test[[;;,1,2]] -> test[[;;,2]] // Thread // DeleteCases[#,x_/;(x//First)==0]&; // AbsoluteTiming
test2*)


(*Cases[test2,x_/;(x//First)==1][[;;,2]];
Table[{Range[%[[1]]//Length]+21 + 1,i}//Transpose,{i,%}] // Transpose // Flatten[#,1]&
Cases[test2,x_/;(x//First)==a][[;;,2]];
Table[{Range[%[[1]]//Length]+21 + 1,i}//Transpose,{i,%}] // Transpose // Flatten[#,1]&
Cases[test2,x_/;(x//First)==b][[;;,2]];
Table[{Range[%[[1]]//Length]+21 + 1,i}//Transpose,{i,%}] // Transpose // Flatten[#,1]&*)
...
(*testList // Map[Cases[#,x_/;(x//First)==1]&] // Part[#,All,All,2]&
(*% // Map[Table[{Range[#[[1]]//Length]+21 + 1,i}//Transpose,{i,#}]&]*)
Table[Table[{Range[(jseedMonomialsOuter // Length)]+1+(jseedMonomialsOuter // Length)*(k-1),i} // Transpose,{i,%[[k]]}] // Transpose // Flatten[#,1]&,{k,1,pList//Length}] // Apply[Join]*)
...
(*need to add the target associations. also make sure that it works for a target list. in this case, will have to shift by more than just 1 probably*)
(*shouldn't be a big deal*)
(*could also not shift and put the equations at the end. probably better to put them at the start however*)
...
finalAns = Merge[{targetAssociation,systemAssociation},Join//Apply](* // Map[Flatten[#,1]&]*); // AbsoluteTiming


nonZeroPositions // Sort;
finalAns // Sort;
%==%%
